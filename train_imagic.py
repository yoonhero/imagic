...
def parse_args():
    parser = argparse.ArgumentParser(description="Imagic Traning Script")
...
def get_full_repo_name(model_id: str, token: Optional[str] = None):
    if token is None:
...
def train():
    ...
    if args.use_8bit_adam:
        try: import bitsandbytes as bnb
            ...
            raise ImportError("Please install the bitsandbytes library")
    ...
    # Optimizer the text embeddings first
    ...
    def train_loop(pbar, optimizer, parmas):
    ...
    progress_bar.set_description("Fine Tuning the diffusion model")
    unet.train()
